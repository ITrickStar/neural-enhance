...
def check_dependencies_pip(requirements_file):
    with open(requirements_file, "r") as f:
        packages = [line.strip() for line in f.readlines() if not line.startswith("#")]

    installed_packages = (
        subprocess.check_output(["pip", "list"]).decode().split("\n")[2:]
    )
    installed_package_names = [package.split()[0] for package in installed_packages if package]

    missing_packages = [
        package for package in packages if package not in installed_package_names
    ]

    if missing_packages:
        print("Следующие пакеты не установлены:")
        for package in missing_packages:
            print(f"- {package}")
    else:
        print("Все пакеты из requirements.txt установлены.")
...
def check_dependencies_yaml(yaml_file):
    ...
    installed_packages = (
        subprocess.check_output(["conda", "list"]).decode("utf-8").split("\n")[3:]
    )
    installed_packages = [
        line.split()[0] for line in installed_packages if line.strip()
    ]
    ...
    if is_all_installed:
        ...
        command_line = " ".join(["python", "src/inference_paired.py"] + args)
        # print(command_line)
        process = subprocess.Popen(
            ["python", "src/inference_paired.py"] + args,
...
if __name__ == "__main__":
    ...
    try:
        check_dependencies_pip(requirements_file)
    except FileNotFoundError as e:
        print(e)
